...
class InsTrialBalanceXlsx(models.AbstractModel):
    ...
    def prepare_report_contents(self, acc_lines, retained, subtotal, filter):
        ...
        self.row_pos += 3
        self.sheet.write_string(self.row_pos, 1, 'Initial Balance', self.format_merged_header)

        self.sheet.write_datetime(self.row_pos, 2, self.convert_to_date(filter.get('date_from')) 
            , self.format_merged_header_without_border)

        self.sheet.write_string(self.row_pos, 3, _(' To '), self.format_merged_header_without_border)

        self.sheet.write_datetime(self.row_pos, 4, self.convert_to_date(filter.get('date_to')),
            self.format_merged_header_without_border)
        
        self.sheet.write_string(self.row_pos, 5, 'Ending Balance', self.format_merged_header)
        ...
        #                         self.format_header)
        self.sheet.write_string(self.row_pos, 5, _('Ending Balance'),
                                self.format_header)
        ...
        if acc_lines:
            if not filter.get('show_hierarchy'):
                for line in acc_lines: # Normal lines
                    ...
                    self.sheet.write_number(self.row_pos, 1, float(acc_lines[line].get('initial_balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 2, float(acc_lines[line].get('debit')), self.line_header_light)
                    self.sheet.write_string(self.row_pos, 3,  '', self.line_header_light)
                    self.sheet.write_number(self.row_pos, 4, float(acc_lines[line].get('credit')), self.line_header_light)
                    ...
                    self.sheet.write_number(self.row_pos, 5, float(acc_lines[line].get('ending_balance')), self.line_header_highlight)
            else:
                for line in acc_lines: # Normal lines
                    self.row_pos += 1
                    ...
                    self.sheet.write_number(self.row_pos, 1, float(line.get('initial_balance')), self.line_header_highlight)
                    self.sheet.write_number(self.row_pos, 2, float(line.get('debit')), self.line_header_light)
                    self.sheet.write_string(self.row_pos, 3,  '', self.line_header_light)
                    ...
                    self.sheet.write_number(self.row_pos, 5, float(line.get('ending_balance')), self.line_header_highlight)
            ...
            if filter.get('strict_range'):
                ...
                self.sheet.write_number(self.row_pos, 1, float(retained['RETAINED'].get('initial_balance')), self.line_header_highlight)
                self.sheet.write_number(self.row_pos, 2, float(retained['RETAINED'].get('debit')), self.line_header_light)
                self.sheet.write_string(self.row_pos, 3,  '', self.line_header_light)
                ...
                self.sheet.write_number(self.row_pos, 5, float(retained['RETAINED'].get('ending_balance')), self.line_header_highlight)
            # Sub total line
            self.row_pos += 2
            ...
            self.sheet.write_number(self.row_pos, 1, float(subtotal['SUBTOTAL'].get('initial_balance')), self.line_header_total)
            self.sheet.write_number(self.row_pos, 2, float(subtotal['SUBTOTAL'].get('debit')), self.line_header_light_total)
            self.sheet.write_string(self.row_pos, 3,  '', self.line_header_light_total)
            ...
            self.sheet.write_number(self.row_pos, 5, float(subtotal['SUBTOTAL'].get('ending_balance')), self.line_header_total)
    ...
    def generate_xlsx_report(self, workbook, data, record):
        ...
        # if not lang:
        #     return True
        #     raise UserError(_('Please Select language in User to Continue... !!'))
